...
def generate_anchors(feat_h, feat_w):
    ...
    x_res = (cfg.PC_X_MAX - cfg.PC_X_MIN) / feat_w

    y_res = (cfg.PC_Y_MAX - cfg.PC_Y_MIN) / feat_h



    xs = np.arange(feat_w) * x_res + cfg.PC_X_MIN + x_res / 2

    ys = np.arange(feat_h) * y_res + cfg.PC_Y_MIN + y_res / 2

    yy, xx = np.meshgrid(ys, xs, indexing="ij")   # [feat_h, feat_w]
...
def compute_loss(cls_pred, reg_pred, gt_boxes, gt_cls, anchors):
    ...
    for b in range(B):
        ...
        if b == 0: print(f"[DEBUG] max_iou max={max_iou.max():.4f} mean={max_iou.mean():.4f} pos={pos_mask.sum().item()} neg={neg_mask.sum().item()}")       boxes.to(cls_pred.device)
